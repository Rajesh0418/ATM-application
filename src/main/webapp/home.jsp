...
<body style="background-image: url('<%= request.getContextPath() %>/pictures/atm_bg.jpeg'); background-size: cover; background-position: center;"  
    ...
    <!-- Right: Logout Button -->
    <form action="<%= request.getContextPath() %>/bank/logout" method="get">
        <button type="submit" class="px-4 py-2 bg-green-500 text-white font-bold rounded-lg hover:bg-red-600 transition duration-300 transform hover:-translate-y-1 hover:scale-110">
            Logout
        </button>
    </form>
</div>
