     ...
     <div class="bg-white/30 backdrop-blur-md p-6 sm:p-10 rounded-xl shadow-2xl w-full max-w-2xl mt-20 ">
       ...
       <form action="<%= request.getContextPath() %>/transactions/withdrawl" method="post" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="accountnumber"  maxlength="12" value="<%= user.getAccountNumber()%>" class="w-full mt-1 p-2 border rounded-md"  required>
            </div>

			<div>
                <label class="block font-semibold">IFSC Code : </label>
                <input type="text" name="ifsccode"  maxlength="11" value="<%= user.getIFSCCode() %>" class="w-full mt-1 p-2 border rounded-md" required>
            </div>
            
            <div>
                <label class="block font-semibold">Amount :</label>
                <input type="number" name="amount" placeholder="Enter how much amount you want to withdraw from an account" class="w-full mt-1 p-2 border rounded-md" required>
